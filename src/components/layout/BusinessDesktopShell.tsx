...
const NAV_ITEMS: NavItem[] = [
  ...
  { path: '/business/post-job', label: 'Post a Job', icon: Plus },
  { path: '/business/candidate-hunter', label: 'Candidate Hunter', icon: Bot },
  { path: '/business/workers', label: 'Workers', icon: Users },
  { path: '/business/intelligence', label: 'Hiring Intelligence', icon: BarChart3 },
  { path: '/business/wallet', label: 'Wallet', icon: Wallet },
  { path: '/business/ratings', label: 'Ratings', icon: Star },
  { path: '/business/profile', label: 'Business Profile', icon: Building2 },
];

interface BusinessDesktopShellProps {
  children: ReactNode;
}

export function BusinessDesktopShell({ children }: BusinessDesktopShellProps) {
      ...
      <aside className="w-60 flex-shrink-0 border-e border-border bg-card flex flex-col h-screen sticky top-0">
        <div className="flex items-center gap-2.5 px-5 h-16 border-b border-border flex-shrink-0">
          ...
          <div className="leading-tight">
            ...
            <p className="text-caption text-muted-foreground">{isRTL ? 'لوحة الأعمال' : 'Business'}</p>
          </div>
        </div>
        ...
        <div className="px-3 py-4 border-t border-border space-y-1">
          ...
          >
            ...
            <span className="truncate">{isRTL ? 'مساعد التوظيف' : 'Hiring Assistant'}</span>
          </button>
          <button
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-y-auto">
        ...
        <header className="h-16 flex-shrink-0 sticky top-0 z-30 border-b border-border bg-background/80 backdrop-blur-sm flex items-center justify-between px-6">
          <div className="flex items-center gap-3 min-w-0">
            <h1 className="text-card-title font-semibold text-foreground truncate">{businessName}</h1>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-52">
              <DropdownMenuItem onClick={() => navigate('/business/profile')}>
                ...
                {isRTL ? 'الملف الشخصي' : 'Business Profile'}
              </DropdownMenuItem>
