      ...
      <div className="space-y-2.5">
        {TALENT_BY_LOCATION.map((loc) => (
          <div key={loc.city} className="flex items-center gap-3">
            ...
            <span className="text-caption font-semibold text-foreground flex-shrink-0 w-14 text-end">
  ...
  const salaryBenchmarks = (
    <section>
      ...
      <div className="rounded-xl border border-border overflow-hidden">
        <div className="grid grid-cols-4 text-caption font-semibold text-muted-foreground uppercase tracking-wider px-3 py-2 bg-muted/50 border-b border-border">
      ...
      <p className="text-caption text-muted-foreground mt-1.5 text-end">SAR/month</p>
    </section>
  );
      ...
      <div className="space-y-2">
        {TIME_TO_FILL.map((t) => (
          <div key={t.category} className="flex items-center gap-2">
            ...
            <span className="text-caption text-foreground flex-1 truncate">{t.category}</span>
            <span className="text-caption font-bold text-foreground">{t.days}d</span>
          </div>
        ))}
      </div>
    </div>
    ...
    <div className="rounded-xl border border-border bg-card p-3">
      ...
      <div className="space-y-1.5 mt-1">
        <div>
          ...
          <p className="text-caption text-muted-foreground">{isRTL ? 'متوسط المنافسة' : 'Avg. competition ratio'}</p>
        </div>
        <div className="pt-1 border-t border-border">
          <p className="text-sm font-bold text-success">↓ 12%</p>
          <p className="text-caption text-muted-foreground">{isRTL ? 'مقارنة بالشهر الماضي' : 'vs. last month'}</p>
        </div>
      </div>
    </div>
  );
      ...
      <div className="space-y-2.5">
        {DEMAND_BY_ROLE.map((d) => (
          <div key={d.role} className="flex items-center gap-3">
            ...
            <span className="text-caption font-semibold text-success flex-shrink-0">{d.trend}</span>
          </div>
        ))}
      </div>
    </section>
        ...
        <div className="flex items-end justify-between h-24 gap-2">
          {SUPPLY_TRENDS.map((s) => (
            <div key={s.month} className="flex-1 flex flex-col items-center gap-1">
              ...
              />
              <span className="text-micro text-muted-foreground">{s.month}</span>
            </div>
          ))}
        </div>
      ...
      <BusinessDesktopShell>
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            ...
            <div className="px-2.5 py-1 rounded-full bg-primary/10 text-primary text-micro font-bold uppercase tracking-wide">
              Live
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-5">
            <div className="card-elevated p-5">{talentByLocation}</div>
            <div className="card-elevated p-5">{salaryBenchmarks}</div>
          ...
          <div className="grid grid-cols-2 gap-5">
            <div className="card-elevated p-5">{demandByRole}</div>
            <div className="card-elevated p-5">{supplyTrends}</div>
          </div>
        </div>
      </BusinessDesktopShell>
    );
    ...
    <MobileLayout footer={<BottomNav />} noPadding>
      ...
      <div className="bg-gradient-to-b from-primary/8 to-transparent px-4 pt-safe pb-2">
        <div className="flex items-center justify-between pt-4 mb-4">
          <div>
            <h1 className="text-page-title text-foreground leading-tight flex items-center gap-2">
              <BarChart3 size={22} className="text-primary" />
              {isRTL ? 'ذكاء التوظيف' : 'Hiring Intelligence'}
            </h1>
